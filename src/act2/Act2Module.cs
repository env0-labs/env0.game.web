...
public sealed class Act2Module : IActModule
{
    ...
    public IEnumerable<OutputLine> Handle(string input, SessionState state)
    {
        ...
        if (_phase == Act2Phase.Booting)
        {
            ...
            {
                _booted = true;
                AddLine(output, "env0.act2 booting");
            ...
            _phase = Act2Phase.Running;

            RenderScene(output, state);
            return output;
        }

        if (_phase == Act2Phase.Running && _repo != null && _gameState != null && _evaluator != null && _executor != null)
        {
